...
const NewsDetailsPage = async ({ params }) => {
  ...
  if (!n) {
    return (
      <div className="flex justify-center items-center mx-auto text-4xl font-bold">
        No news found
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <ul className="flex items-center gap-3 ">
          <Link href={`/category/${n.category_id}`}>
            <button className="text-white bg-red-500 px-2 rounded-sm font-semibold  flex items-center">
              <FaArrowLeft />
              See other news
            </button>
          </Link>
